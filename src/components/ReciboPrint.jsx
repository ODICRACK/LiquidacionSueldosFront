...
export const ReciboPrint = React.forwardRef(({ data }, ref) => {
    ...
    const getBase = (item) => {
        if (item.tipo === 'PORCENTAJE' && item.porcentaje && parseFloat(item.porcentaje) !== 0) {
            // Deduce el valor base matemáticamente: Base = Monto / (Porcentaje / 100)
    ...
    const calcularAntiguedad = () => {
        ...
        // Extrae el año del periodo (Ej: "04/2026" -> 2026)
        const anioActual = liquidacion.periodo.split('/')[1] ? parseInt(liquidacion.periodo.split('/')[1]) : new Date().getFullYear();
        return Math.max(0, anioActual - anioIngreso);
    };
    ...
    const colorBgAzulOscuro = '#1a3b8a';
    const colorBgAzulClaro = '#5b74a6';
    
    const colHeadStyle = { background: colorBgAzulClaro, color: '#fff', padding: '4px', textAlign: 'center', border: '1px solid #000', fontSize: '11px' };
    ...
    const renderItemRow = (item, idx) => (
        <tr key={idx} style={rowStyle}>
            ...
            <td style={cellRight}>{formatMoney(item.valor_ingresado)}</td>
        </tr>
    );
    ...
    return (
        <div ref={ref} style={{ padding: '20px 40px', background: '#fff', color: '#000', fontFamily: fontMain, maxWidth: '900px', margin: '0 auto' }}>
            ...
            <div style={{ border: '2px solid #000', padding: '8px 12px', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase' }}>
                ...
                <div>Tierra del Fuego e Islas del Atlántico Sur</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', border: '2px solid #000', borderTop: 'none', fontSize: '11px' }}>
                <tbody>
                    <tr style={{ background: colorBgAzulOscuro, color: '#fff', fontWeight: 'bold', textAlign: 'center' }}>
                    ...
                    <tr style={{ textAlign: 'center', fontWeight: 'bold', background: '#f8f9fa' }}>
                        <td colSpan={2} style={{ padding: '4px', border: '1px solid #000' }}>{liquidacion.periodo}</td>
                        ...
                        <td style={{ padding: '4px', border: '1px solid #000' }}>{calcularAntiguedad()} años</td>
                    </tr>
                    <tr style={{ background: colorBgAzulOscuro, color: '#fff', fontWeight: 'bold', textAlign: 'center' }}>
                        ...
                        <td style={{ padding: '4px', border: '1px solid #000' }}>F. PAGO APORTES</td>
                    </tr>
                    <tr style={{ textAlign: 'center', fontWeight: 'bold', background: '#f8f9fa' }}>
                        <td colSpan={2} style={{ padding: '4px', border: '1px solid #000' }}>{empleado.fecha_ingreso ? empleado.fecha_ingreso.split('T')[0] : '-'}</td>
                        <td colSpan={2} style={{ padding: '4px', border: '1px solid #000' }}>Administrativo A</td>
                        <td style={{ padding: '4px', border: '1px solid #000' }}>{empleado.cuil}</td>
                        <td style={{ padding: '4px', border: '1px solid #000' }}>BNA</td>
                        <td style={{ padding: '4px', border: '1px solid #000' }}>Pendiente</td>
                    </tr>
                </tbody>
            </table>

            {/* Bloque 3: Costos del Empleador (Informativos) */}
            <div style={{ background: colorBgAzulOscuro, color: '#fff', display: 'flex', justifyContent: 'space-between', padding: '4px 10px', fontWeight: 'bold', border: '2px solid #000', borderTop: 'none', borderBottom: 'none', fontSize: '12px' }}>
            ...
            {/* Bloque 4: Remuneraciones y Descuentos */}
            <div style={{ background: colorBgAzulOscuro, color: '#fff', textAlign: 'center', padding: '4px 10px', fontWeight: 'bold', border: '2px solid #000', borderTop: 'none', borderBottom: 'none', fontSize: '12px' }}>
                SUELDO BRUTO
            </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', border: '2px solid #000', borderTop: 'none' }}>
                ...
                <tbody>
                    {/* Grupo REMUNERATIVOS */}
                    <tr>
                        <td colSpan={4} style={{ textAlign: 'center', fontWeight: 'bold', background: '#e9e9e9', padding: '4px', fontSize: '11px' }}>REMUNERATIVOS</td>
                    </tr>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', border: '2px solid #000', borderTop: 'none', fontSize: '11px', fontWeight: 'bold' }}>
                <tbody>
                    <tr style={{ background: '#fff' }}>
                        <td style={{ padding: '6px', borderRight: '1px solid #000', width: '25%', textTransform: 'uppercase' }}>RESUMEN SALARIAL</td>
                        <td style={{ padding: '6px', textAlign: 'right' }}>Remunerativos:</td>
            ...
            <div style={{ background: colorBgAzulOscuro, color: '#fff', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '6px', fontWeight: 'bold', border: '2px solid #000', borderTop: 'none', fontSize: '16px', position: 'relative' }}>
            ...
            <div style={{ marginTop: '50px', display: 'flex', justifyContent: 'flex-end', paddingRight: '50px' }}>
                <div style={{ borderTop: '1px dashed #000', width: '250px', textAlign: 'center', paddingTop: '5px', fontSize: '12px', fontWeight: 'bold' }}>
                    Firma del Empleado
                </div>
            </div>

            {/* Bloque 7: Gráfico y Aclaraciones */}
            <div style={{ display: 'flex', marginTop: '40px', borderTop: '2px solid #000', paddingTop: '15px' }}>
                <div style={{ flex: 1, paddingRight: '20px', fontSize: '10px', color: '#333' }}>
                    <p style={{ fontWeight: 'bold', color: '#000', fontSize: '12px', margin: '0 0 10px 0' }}>Detalle de la composición salarial</p>
                            ...
                            options={{ 
                                ...
                                plugins: { 
                                    legend: { 
                                        display: true,
                                        position: 'right',
                                        labels: { boxWidth: 12, font: { size: 10, family: fontMain } }
                                    } 
                                } 
                            }} 
                        />
